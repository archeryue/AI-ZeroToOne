"""
Evaluate NNUE v2 with C++ alpha-beta search against various opponents.

Usage:
    python eval_nnue_v2.py [--games 100] [--depth 4] [--opponent all]
    python eval_nnue_v2.py --nnue_weight 0.8  # test with 80% NNUE + 20% material
"""
...
from engine_c._xiangqi import (
    Board, Game, NNUESearchV2, get_legal_moves,
...
def evaluate(search_engine, depth, opponent_name, opponent_fn, num_games=100):
    ...
    print(f"\n=== NNUE v2 (depth {depth}) vs {opponent_name} ({num_games} games) ===")
...
def main():
    ...
    parser.add_argument('--weights', type=str,
                        default=os.path.join(SCRIPT_DIR, 'checkpoints', 'nnue_v2_weights.bin'))
    parser.add_argument('--nnue_weight', type=float, default=1.0,
                        help='NNUE blend weight (1.0=pure NNUE, 0.0=pure material)')
    args = parser.parse_args()

    print(f"Loading NNUE v2 weights: {args.weights}")
    search = NNUESearchV2()
    ok = search.load_weights(args.weights)
    if not ok:
        print("ERROR: Failed to load weights")
        return
    search.set_nnue_weight(args.nnue_weight)
    print(f"Search depth: {args.depth}")
    print(f"NNUE weight: {args.nnue_weight}")
