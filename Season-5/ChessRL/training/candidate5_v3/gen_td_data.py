"""
Generate TD training data via self-play using the v2 NNUE C++ engine.

Same pipeline as v2's gen_td_data.py, but uses the stronger v2 engine
for self-play, producing higher quality search scores.
...
def play_game_with_scores(args_tuple):
    """
    Play a self-play game using v2 engine (worker function for multiprocessing).

    Noise schedule:
      - Moves 1-6: fully random (diverse openings, no search)
    ...
    from engine_c._xiangqi import (
        Game, NNUESearchV2, get_legal_moves,
    ...
    engine.set_nnue_weight(1.0)  # pure NNUE, no material blending
...
def main():
    parser = argparse.ArgumentParser(description="Generate TD training data via v2 self-play")
    ...
    parser.add_argument('--weights', type=str,
                        default=os.path.join(PROJECT_DIR, 'training/candidate5_v2/checkpoints/nnue_v2_weights.bin'),
                        help='Path to v2 NNUE weights')
    ...
    print(f"Engine: NNUESearchV2 (pure NNUE)")
