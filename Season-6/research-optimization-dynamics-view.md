...
梯度流 $\dot\theta = -\nabla L$ 是一个**纯粹的当下系统**——每一步只看当前梯度，完全不记得自己从哪来、之前往哪走。

这在损失曲面是"狭长山谷"的时候会出大问题。想象一个细长的抛物面槽，最低点在槽底中轴线上：

- 梯度流在槽里会**反复横跳**：每一步梯度都指向两侧壁（横向坡更陡），沿轴方向的分量反而小，结果在横向来回震荡，轴向进展极慢。
- 深层原因：**条件数**（最大 / 最小曲率之比）越大，梯度流收敛越慢，因为它每步都在被"最陡的那个方向"主导。

#### 想要的性质：惯性

解法的直觉是：让系统有**记忆**，能沿历史一致的方向"积累速度"，在来回震荡的方向上相互抵消。

物理上，这正是**惯性**的含义。一个有质量的小球从山坡滚下，不是每时每刻都只听从当下的坡度——它会保留之前积累的动能继续往前冲，横向的小震荡会被动能抹平。

这个性质在梯度流里是根本缺失的：梯度流对应的是质量为零的粒子，当下受力直接决定当下速度，没有任何"积累"可言。想引入惯性，就必须**换一套物理模型**。

#### 新模型：牛顿第二定律

给 $\theta$ 赋予质量，写出 $F = ma$（取质量 = 1）：

$$\ddot\theta = -\nabla L(\theta)$$

但纯粹的牛顿系统没有耗散，小球会在谷底永远震荡而不停下。加上与速度成正比的**摩擦阻力** $-\lambda\dot\theta$（保证系统耗散能量、最终收敛）：
